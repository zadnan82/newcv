...
import { getParisStyles } from './ParisStyles';
import { useTranslation } from 'react-i18next';

/**
 * Paris Template Implementation
 * Elegant French-inspired design with artistic touches
 */
const Paris = ({ formData, customSettings, darkMode }) => {
  ...
  const settings = {
    accentColor: customSettings?.accentColor || '#9c27b0',
  ...
  const styles = getParisStyles(darkMode, settings);
  ...
  return (
    <div style={styles.container}>
      {/* Decorative header */}
      <div style={styles.decorativeHeader}></div>
                ...
                <div style={styles.sectionContent}>
                  {data.educations.map((edu, index) => (
                    <div key={index} style={styles.educationItem}>
                      ...
                      {edu.gpa && (
                        <div style={styles.educationGpa}>{t('resume.education.gpa')}: {edu.gpa}</div>
                      )}
                    </div>
                  ))}
                </div>
                    ...
                    Array.isArray(data.referrals) && data.referrals.map((reference, index) => (
                      <div key={`ref-${index}`} style={styles.referenceItem}>
                        ...
                        <div style={styles.referenceContact}>
                          {reference.email && <div style={styles.referenceEmail}>{reference.email}</div>}
                          {reference.phone && <div style={styles.referencePhone}>{reference.phone}</div>}
                        </div>
                      </div>
                    ))
                  )}
          ...
          <div style={styles.sidebar}>
                    ...
                    {data.languages.map((language, index) => (
                      <div key={index} style={styles.languageItem}>
                        ...
                        <div style={styles.languageProficiency}>
                          <div style={styles.languageBadge}>{language.proficiency}</div>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {hasCustomSections() && 
              data.custom_sections.map((section, sectionIndex) => (
                <div key={`custom-${sectionIndex}`} style={styles.sideSection}>
                  <h2 style={styles.sideSectionHeading}>
