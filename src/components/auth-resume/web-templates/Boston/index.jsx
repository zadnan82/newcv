...
import { getBostonStyles } from './BostonStyles';
import { useTranslation } from 'react-i18next';

/**
 * Boston Template Implementation
 * Sophisticated, classic design with subtle accents and generous whitespace
 */
const Boston = ({ formData, customSettings, darkMode }) => {
  ...
  const settings = {
    accentColor: customSettings?.accentColor || '#9c7c38',
    fontFamily: customSettings?.fontFamily || "'Cormorant Garamond', 'Georgia', serif",
  ...
  // Get the styles
  const styles = getBostonStyles(darkMode, settings);
  ...
  const hasExperiences = () => {
    return data.experiences && Array.isArray(data.experiences) && data.experiences.filter(exp => exp && exp.company).length > 0;
  };

  const hasEducation = () => {
    return data.educations && Array.isArray(data.educations) && data.educations.filter(edu => edu && edu.institution).length > 0;
  };

  const hasInternships = () => {
    return data.internships && Array.isArray(data.internships) && data.internships.filter(intern => intern && intern.company).length > 0;
  };

  const hasCourses = () => {
    return data.courses && Array.isArray(data.courses) && data.courses.filter(course => course && course.name).length > 0;
  };

  const hasSkills = () => {
    return data.skills && Array.isArray(data.skills) && data.skills.filter(skill => skill && skill.name).length > 0;
  };

  const hasLanguages = () => {
    return data.languages && Array.isArray(data.languages) && data.languages.filter(lang => lang && lang.language).length > 0;
  };
  ...
  const hasExtracurriculars = () => {
    return data.extracurriculars && Array.isArray(data.extracurriculars) && data.extracurriculars.filter(activity => activity && activity.title).length > 0;
  };

  const hasCustomSections = () => {
    return data.custom_sections && Array.isArray(data.custom_sections) && data.custom_sections.filter(section => section && section.title && (section.content || (section.items && section.items.length > 0))).length > 0;
  };

  const hasReferences = () => {
    if (!data.referrals) return false;
    if (data.referrals.providedOnRequest === true) return true;
    return Array.isArray(data.referrals) && data.referrals.filter(ref => ref && ref.name).length > 0;
  };
          ...
          {hasExperiences() && (
            <div style={styles.section}>
              ...
              {data.experiences.map((exp, index) => (
                <div key={index} style={{
                  ...styles.expItem,
                  ...(index === data.experiences.length - 1 ? styles.lastItem : {})
                }}>
          ...
          {hasEducation() && (
            <div style={styles.section}>
              ...
              {data.educations.map((edu, index) => (
                <div key={index} style={{
                  ...styles.expItem,
                  ...(index === data.educations.length - 1 ? styles.lastItem : {})
                }}>
                  <div style={styles.expHeader}>
          ...
          {hasInternships() && (
            <div style={styles.section}>
              ...
              {data.internships.map((internship, index) => (
                <div key={index} style={{
                  ...styles.expItem,
                  ...(index === data.internships.length - 1 ? styles.lastItem : {})
                }}>
                  <div style={styles.expHeader}>
          ...
          {hasCourses() && (
            <div style={styles.section}>
              ...
              {data.courses.map((course, index) => (
                <div key={index} style={{
                  ...styles.expItem,
                  ...(index === data.courses.length - 1 ? styles.lastItem : {})
                }}>
                  <div style={styles.expHeader}>
          ...
          {hasExtracurriculars() && (
            <div style={styles.section}>
              ...
              {data.extracurriculars.map((activity, index) => (
                <div key={index} style={{
                  ...styles.expItem,
                  ...(index === data.extracurriculars.length - 1 ? styles.lastItem : {})
                }}>
                  <div style={styles.expHeader}>
              ...
              <div key={`custom-${sectionIndex}`} style={styles.section}>
                ...
                {section.items && section.items.length > 0 ? (
                  section.items.map((item, itemIndex) => (
                    <div key={`custom-item-${sectionIndex}-${itemIndex}`} style={{
                      ...styles.expItem,
                  ...
                  <div style={styles.summary}>{section.content}</div>
                )}
              </div>
            ))
          }
        ...
        <div style={styles.sidebar}>
          ...
          {hasPersonalDetails() && (
            <div style={styles.sideSection}>
              <h3 style={styles.sideSectionTitle}>
              ...
              <div style={styles.personalInfoGrid}>
                {data.personal_info.address && (
                  <div style={styles.personalInfoItem}>
                    <span style={styles.personalInfoIcon}>🏠</span>
              ...
              <div>
                {data.languages.map((lang, index) => (
                  <div key={index} style={styles.languageItem}>
                    ...
                    <div style={styles.languageProficiency}>{lang.proficiency}</div>
                  </div>
                ))}
              </div>
            </div>
